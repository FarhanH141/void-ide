...
export const agentSystemMessage = ({
	...
	const toolDefinitions = includeXMLToolDefinitions
		? systemToolsXMLPrompt("agent", mcpTools)
		: null;
...
${
	mode === "gather"
		? `Your job is to understand the user's request, gather information, and formulate a clear plan. You must outline the steps you will take and ask for confirmation before proceeding.`
		: mode === "normal"
		? `Your job is to assist the user with their coding tasks.`
		: ""
}
...
${directoryStr}
	...
	if (mode === "gather") {
		details.push(
			`You MUST create files when they don't exist and edit existing files when requested. Do NOT ask for permission - take immediate action using file creation and editing tools.`
		);
		details.push(
			`When users ask you to implement features, fix bugs, or make changes, you should IMMEDIATELY start creating or editing the necessary files. Be proactive about file operations.`
		);
		details.push(
			`You have access to tools that are executed immediately upon your request. You can use one tool per message, and will receive the result of that tool use in the user's response. You use tools step-by-step to accomplish a given task, with each tool use informed by the result of the previous tool use.`
		);
		details.push(
			`You must be proactive and take initiative. When the user asks you to analyze their codebase, explore project structure, or understand their code, you should immediately start using tools without asking for permission or providing lengthy explanations first.`
		);
		details.push(
			`If you encounter truncated outputs or incomplete information from tools, immediately use alternative approaches like terminal commands to get complete information. Never complain to the user about limitations - always find a solution.`
		);
		details.push(
			`When analyzing a codebase, start by getting the directory structure with get_dir_tree or ls_dir, then read key files like README.md, package.json, configuration files, and main entry points. Be thorough in your exploration.`
		);
		details.push(
			`You should use tools step-by-step to accomplish tasks. Each tool use should be informed by the results of previous tool uses.`
		);
	...
	if (mode !== "agent") {
		details.push(
			"Your instructions are to be followed precisely. Here is how you will operate:"
		);
		details.push(
			"1. THINK: First, think step-by-step and formulate a brief plan to address the user's request."
		);
		details.push(
			"2. EXPLAIN: Briefly explain your plan to the user. Do not ask for permission, just state what you are about to do."
		);
		details.push(
			"3. EXECUTE: Use the available tools to execute your plan. Use one tool at a time."
		);
		details.push(
			"4. SUMMARIZE: After you are finished, provide a concise summary of the changes you have made."
		);
		...
		details.push(
			`You MUST use the exact tool names from the following list: ${toolList.join(
				", "
			)}.`
		);
...
`;
		...
		details.push(
			"Always use the most appropriate tools for the task. Be proactive and take initiative."
		);
		details.push(
			"Ensure your code is complete and includes necessary imports and dependencies."
		);
		details.push(
			"Follow existing code conventions and patterns within the user's project."
		);
	}

	if (mode === "gather") {
		details.push(
			`You are in 'gather' mode. Your goal is to understand the user's request, gather information, and formulate a clear plan.`
		);
		details.push(
			`You MUST outline the steps you will take and ask for confirmation before using any tools to modify files or run commands.`
		);
	}
...
export const messageOfSelection = async (
	...
	if (s.type === "File" || s.type === "CodeSelection") {
		const { val } = await readFile(
			opts.fileService,
			s.uri,
			DEFAULT_FILE_SIZE_LIMIT
		);
		const lineNumAdd =
			s.type === "CodeSelection" ? lineNumAddition(s.range) : "";
		const content =
			val === null
				? "null"
		...
		const dirStr: string = await opts.directoryStrService.getDirectoryStrTool(
			s.uri
		);
		...
		const strOfFiles = await Promise.all(
			uris.map(async (uri) => {
				const { val, truncated } = await readFile(
					opts.fileService,
					uri,
					opts.folderOpts.maxCharsPerFile
				);
